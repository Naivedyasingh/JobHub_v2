...
from utils.applications import (
    get_job_applications,
    update_application_status,
    accept_application,  # ← NEW IMPORT
    reject_application,  # ← NEW IMPORT
)
...
class ApplicationDataManager:
    ...
    def accept_application_with_job_update(self, app_id):  # ← NEW METHOD
        """Accept application and update job hiring status."""
        return accept_application(app_id)
    
    def reject_application_with_update(self, app_id):  # ← NEW METHOD
        """Reject application with proper status update."""
        return reject_application(app_id)
...
class ApplicationCardRenderer:
    ...
    def render_action_buttons(self, app, data_manager):
        ...
        # ← UPDATED: Use new methods that update job status
        if c1.button("✅ Accept", key=f"acc{app['id']}", use_container_width=True):
            success, message = data_manager.accept_application_with_job_update(app["id"])
            if success:
                st.success(f"✅ Accepted! {message}")
            else:
                st.error(f"❌ {message}")
            st.rerun()
            
        if c2.button("❌ Reject", key=f"rej{app['id']}", use_container_width=True):
            success, message = data_manager.reject_application_with_update(app["id"])
            if success:
                st.info("❌ Rejected")
            else:
                st.error(f"❌ {message}")
